...
function buildEmbedUrl(input) {
  ...
  // Loose YouTube id recovery for compact malformed strings like 'httpsyoutubeFgU6Qghbyc'
  const looseYt = candidate.match(/[A-Za-z0-9_-]{11}/);

  let dmId = extractDailymotionId(candidate);
  let ytId = extractYouTubeId(candidate) || (looseYt ? looseYt[0] : null);

  // Sanity-check dmId and ytId
  ...
  if (dmId) {
    ...
    // If the raw input was a full URL, only accept it when it's a known supported host
    try {
      const u = raw.startsWith('http') ? new URL(raw) : new URL(raw, 'https://example.com');
      const host = u.hostname.toLowerCase();
      if (host.includes('dailymotion.com') || host.includes('dai.ly') || host.includes('youtube.com') || host.includes('youtu.be')) {
        // Use the normalized host path where possible, but avoid embedding arbitrary pages
        if (host.includes('dailymotion.com') || host.includes('dai.ly')) {
          // Try to extract an ID from the path
          const dmCandidate = extractDailymotionId(u.pathname || '');
          if (dmCandidate) {
            baseUrl = `https://www.dailymotion.com/embed/video/${dmCandidate}`;
            service = 'dailymotion';
          }
        }
        if (!baseUrl && (host.includes('youtube.com') || host.includes('youtu.be'))) {
          const maybeYt = extractYouTubeId(raw);
          if (maybeYt) {
            baseUrl = `https://www.youtube.com/embed/${maybeYt}`;
            service = 'youtube';
          }
        }
      } else {
