...
function buildEmbedUrl(input) {
  ...
  // Robustness: sanitize obviously malformed inputs (like 'httpsyoutube...')
  if (!input || typeof input !== 'string') return '';
  const raw = input.trim();

  // If someone accidentally stored an iframe path or an embed path, try to extract the id portion
  let candidate = raw;
  if (candidate.includes('/embed/video/')) {
    candidate = candidate.split('/embed/video/').pop().split('?')[0];
  } else if (candidate.includes('embed/video/')) {
    candidate = candidate.split('embed/video/').pop().split('?')[0];
  }

  // Prefer host-based detection: if input mentions Dailymotion use Dailymotion; if YouTube mention use YouTube
  const looseYt = candidate.match(/[A-Za-z0-9_-]{11}/);
  let dmId = null;
  let ytId = null;
  let baseUrl = '';
  let service = null;
  ...
  try {
    // If it clearly references Dailymotion hosts, extract DM id first
    if (lowered.includes('dailymotion.com') || lowered.includes('dai.ly')) {
      dmId = extractDailymotionId(candidate);
    }
    // If it clearly references YouTube hosts, extract YouTube id
    if (lowered.includes('youtube.com') || lowered.includes('youtu.be')) {
      ytId = extractYouTubeId(candidate);
    }
  } catch (e) {
    // ignore
  }

  // If no host hint, fall back to bare-id rules
  if (!dmId && /^x[0-9a-zA-Z]+$/.test(candidate)) dmId = candidate;
  if (!ytId && /^[A-Za-z0-9_-]{11}$/.test(candidate)) ytId = candidate;

  // If still nothing, try looser extraction: prefer Dailymotion when its pattern matches
  if (!dmId && !ytId) {
    dmId = extractDailymotionId(candidate);
    ytId = extractYouTubeId(candidate) || (looseYt ? looseYt[0] : null);
  }

  // Final sanity checks
  if (dmId && !/^x[0-9a-zA-Z]{3,}$/.test(dmId)) dmId = null;
  if (ytId && !/^[A-Za-z0-9_-]{11}$/.test(ytId)) ytId = null;
  ...
  if (dmId) {
    ...
    // Not a recognized provider or malformed string — refuse to embed
    return '';
  }
