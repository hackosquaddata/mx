...
export default function PaymentPage() {
	...
	const helpWhatsappUrl = useMemo(() => {
		...
		const msg = encodeURIComponent(
			...
			`Name: ${form.name || ''}%0A` +
			// If free course, show only enroll button and skip payment UI
			if (isFreeCourse) {
				return (
					<div className="flex min-h-screen bg-gradient-to-br from-[#0a0f14] via-[#0a0f14] to-black text-slate-100">
						...
						<div className="flex-1 flex flex-col items-center justify-center">
							<div className="max-w-md w-full bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
								...
								<p className="mb-6 text-slate-300">No payment required. Click below to enroll instantly and start learning.</p>
								<button
			...
			// Paid course: show normal payment UI
			return (
				<div className="flex min-h-screen bg-gradient-to-br from-[#0a0f14] via-[#0a0f14] to-black text-slate-100">
					<Sidebar />
					<div className="flex-1 p-6">
						<div className="max-w-6xl mx-auto">
							{/* ...existing payment UI and sidebar... */}
						</div>
					</div>
				</div>
			);
